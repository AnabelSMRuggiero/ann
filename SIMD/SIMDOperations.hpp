...
At the time of writting, this code is unreleased and not published under a
license. As a result, I currently retain all legal rights I am legally entitled
to.

I am currently considering a permissive license for releasing this code, such as
the Apache 2.0 w/LLVM exception. Please refer to the project repo for any
updates regarding liscensing. https://github.com/AnabelSMRuggiero/NNDescent.cpp
*/
...
concept VectorRequirements = std::same_as<UnderlyingType, std::remove_const_t<typename DataVector::UnderlyingType>> &&
    std::same_as<Intrinsic, typename DataVector::VectorType>;

template <InstructionSet First, InstructionSet... Rest>
consteval InstructionSet SelectInstructions() {
    auto equalToFirst = [](auto next) -> bool { return (First == next); };
    auto all = [](auto... bools) { return (... && bools); };
...
template <typename VectorType, typename... VectorTypes>
    requires(
        VectorRequirements<VectorTypes, std::remove_const_t<typename VectorType::UnderlyingType>, typename VectorType::VectorType> &&...)
...
struct Zero : Op<0> {
    static constexpr std::tuple validInstructionSets{
        InstructionSet::avx, InstructionSet::avx2, InstructionSet::fma, InstructionSet::avx512
    };
...
struct FMA : Op<3> {
    static constexpr std::tuple validInstructionSets{ InstructionSet::fma, InstructionSet::avx512 };
