...
struct vector_span_iterator{
    ...
    std::ptrdiff_t operator-(vector_span_iterator other){
        return (currentElement - other.currentElement)/elementsPerVector;
    }
    
    bool operator==(vector_span_iterator other){
        return currentElement == other.currentElement;
    }
    
    reference operator*(){
        return reference{MakeAlignedPtr(currentElement, *this)};
    }
    ...
    auto operator<=>(vector_span_iterator& rhs){
        return currentElement<=> rhs.currentElement;
    }
};
...
struct vector_span{
    ...
    nnd::AlignedSpan<ElementType, alignment> excess() {
        return nnd::AlignedSpan<ElementType, alignment>{nnd::MakeAlignedPtr(data + fullExtent - fullExtent % elementsPerVector, *this), fullExtent % elementsPerVector};
    }
